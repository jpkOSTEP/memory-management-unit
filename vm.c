...
void set_physical_mem() {
    ...
    //calculate the number of physical and virtual pages and allocate
...
/*
 * Add a virtual to physical page translation to the TLB.
 */
int
...
print_TLB_missrate()
{
    ...
    /*Code here to calculate and print the TLB miss rate*/
    printf("TLB hit: %d\nTLB miss: %d\n", tlb_hit, tlb_miss);
...
pte_t *translate(pde_t *pgdir, void *va) {
    /* Get the Page directory index (1st level) Then get the
    * 2nd-level-page table index using the virtual address.  Using the page
    * directory index and page table index get the physical address.
    *
    * Check the TLB before performing the translation. If
...
int page_map(pde_t *pgdir, void *va, void *pa)
{
    ...
    /*Similar to translate(), find the page directory (1st level)
    ...
    if (page_dir[page_dir_index] == NULL) {
        ...
        // printf("Assigned physical loc %lu -> %lu for page table\n", (page_dir[page_dir_index]), (void*)(page_dir[page_dir_index])+PGSIZE);
...
void *t_malloc(unsigned int num_bytes) {
   ...
   /* 
    * If the page directory is not initialized, then initialize the
    ...
    */
    ...
    for(int i = 0; i < pages_required; i++){
        // printf("Assigning %lu -> %lu\n", (unsigned long)phy_page, (unsigned long)(phy_page+PGSIZE));
...
void t_free(void *va, int size) {
    ...
    /* Free the page table entries starting from this virtual address
     * (va). Also mark the pages free in the bitmap. Perform free only if the 
     * memory from "va" to va+size is valid.
     *
     * Also, remove the translation from the TLB
     */
    pthread_mutex_lock(&mutex);
    if (memory_start == NULL) {
            printf("Memory not initialised\n");
            pthread_mutex_unlock(&mutex);
            return;
        }
    ...
    for(int i = 0; i < pages_free; i++) {
        if (page_table[page_table_index+i] == NULL) {
            printf("Invalid address to free 1\n");
            pthread_mutex_unlock(&mutex);
            return;
        }
    }
    ...
    // printf("Freeing %lu -> %lu\n", (unsigned long)physical_addr, (unsigned long)physical_addr+pages_free*PGSIZE);
    for(int i = 0; i < pages_free; i++) {
...
void put_value(void *va, void *val, int size) {
    ...
    /* Using the virtual address and translate(), find the physical page. Copy
    ...
    if (PGSIZE - offset > size) {
        ...
        if (pt_entry == NULL){
            printf("Invalid address to put value\n");
            return;
        }
        ...
        while (size_copy > 0) {
            ...
            if (pt_entry == NULL){
                printf("Invalid size/address to put value\n");
                pthread_mutex_unlock(&mutex);
                return;
            }
            physical_address = *pt_entry;
...
void get_value(void *va, void *val, int size) {
    ...
    /* put the values pointed to by "va" inside the physical memory at given
    ...
    */
    ...
    if (PGSIZE - offset > size) {
        pte_t* pt_entry = translate(page_dir, va);
        if (pt_entry == NULL){
            printf("Invalid address to get value\n");
            return;
        }
        physical_address = *pt_entry;
        physical_address += offset;
        ...
        while (size_copy > 0) {
            pte_t* pt_entry = translate(page_dir, va_copy);
            if (pt_entry == NULL){
                printf("Invalid size/address to get value\n");
                pthread_mutex_unlock(&mutex);
                return;
            }
            physical_address = *pt_entry;
...
void mat_mult(void *mat1, void *mat2, int size, void *answer) {
    ...
    /* You will index as [i * size + j] where  "i, j" are the indices of the
...
void calculate_entries_bits(){
    ...
    if (page_table_bits > remaining_bits){
        ...
        if (page_table_entries > pow(2, bits_per_level)) {
            page_table_entries = pow(2, bits_per_level);
        }
    ...
    if (page_dir_entries > pow(2, remaining_bits)) {
            page_dir_entries = pow(2, remaining_bits);
        }
}
